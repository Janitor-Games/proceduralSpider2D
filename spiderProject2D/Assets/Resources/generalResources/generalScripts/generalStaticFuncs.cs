...
public static class generalStaticFuncs
{
    ...
    public static RaycastHit2D fireCurvedRay(Vector3 up,Vector3 forward, float direction, Transform castPosition, float castRad, float castAngle, float castArc, int level, Color dbgColor1,Color dbgColor2)
    {
        ...
        for (int x = 0; x < iterations; x++)
        {
            ...
            Debug.DrawLine(castPosition.position + lineS, castPosition.position + lineE, x==0?dbgColor2:dbgColor1);
    ...
    //set gib direction vectors to the same vector
    public static void setGibsVector(GameObject gibs, Vector3 vec)
    {
        foreach(Transform gib in gibs.transform)
        {
            gib.GetComponent<gibScript>().gibVector=vec;
        }
    }
}
