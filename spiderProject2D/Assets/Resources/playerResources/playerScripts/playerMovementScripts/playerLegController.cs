...
public class playerLegController : MonoBehaviour
{
    ...
    private playerBodyController pb => legData.pb;
    ...
    private Vector2 findLiftPosition()
    {
        ...
        Vector2 liftTarget = halfway + pb.playerUpVector * legData.stepHeight;
        return liftTarget;
    }
    ...
    private bool processCurvedRay()
    {
        var hit = generalStaticFuncs.fireCurvedRay(pb.playerUpVector, transform.forward, castDirection, legData.legCast, legData.castRad, legData.castAngle, legData.castArc, legData.level, Color.green, Color.gold);
        var coll = hit.collider;
        if (coll != null) finalTarget = hit.point;
        return coll != null;
    }
    ...
    {
        transform.position = Vector3.MoveTowards(transform.position, target, Time.fixedDeltaTime * legData.stepSpeed);
    }
